...
export function ConsumerMeals({}: ConsumerMealsProps) {
  const [foodItems, setFoodItems] = useState<IFoodEntries[]>();
  const [loading, setLoading] = useState(true);
  const today = new Date().toDateString();
  const todayMeals = useMemo(
    () =>
      foodItems?.length &&
      foodItems.filter((item) => new Date(item.date).toDateString() === today),
    [foodItems]
  );
  ...
  const fetchMyMeals = async () => {
    try {
      const res = await authFetch(`/my/entries`, {
        method: "GET",
      });
      setFoodItems(res);
    } catch (e: any) {
                    ...
                    <View key={date} style={styles.dateGroup}>
                      ...
                      {mealsByDate[date].map((meal) => (
                        <View key={meal.id} style={styles.historyMeal}>
                          ...
                          <Text style={styles.historyMealName}>
                            {meal.food_name}
                          </Text>
